...
function DaysDisplay({ days, stock }: { days: number; stock: number }) {
    ...
    if (days <= 30) return <span className="text-indigo-700 font-semibold text-xs">{Math.round(days)} hari</span>;
    ...
    return (
        <div className="space-y-6 sm:space-y-8 animate-fade-in max-w-7xl mx-auto pb-10">
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <div className="flex items-center gap-2">
                                ...
                                <span className="px-2.5 py-0.5 rounded-full bg-indigo-50 text-indigo-700 border border-indigo-200 text-[10px] font-extrabold">
                                    BOM Forecast
                                </span>
                            </div>
                            {projection && (
                                                ...
                                                return (
                                                    <div key={item.product_id} className="contents">
                                                        ...
                                                        {isExp && item.day_breakdown && item.day_breakdown.map((bd, bi) => {
                                                            ...
                                                            return (
                                                                <tr key={`bd-${item.product_id}-${bi}`} className="bg-indigo-50/40 border-b border-indigo-100/60">
            ...
            <div className="bg-white/85 backdrop-blur-xl rounded-3xl shadow-[0_4px_20px_-4px_rgba(0,0,0,0.04)] border border-slate-200/80 p-5 sm:p-6 space-y-4">
                ...
                <div className="flex items-center gap-2.5 p-3 rounded-2xl bg-gradient-to-r from-blue-50/80 to-indigo-50/60 border border-blue-200/60 text-xs text-blue-900 font-medium">
                        ...
                        <table className="w-full text-xs sm:text-sm">
                                    ...
                                    {[
                                        ...
                                        >
                                            <div className="flex items-center gap-1">
                                                <span>{col.label}</span>
                                                {col.key === sortCol && (
                                                    <span className="text-indigo-600 font-bold">{sortAsc ? "▲" : "▼"}</span>
                                                )}
                                            </div>
                                        </th>
                                    ))}
                                                ...
                                                {editingMinStockId === p.id ? (
                                                    <div className="flex items-center justify-end gap-1.5">
                                                        <input 
                                                            ...
                                                            className="w-20 px-2 py-1 border border-indigo-300 rounded-lg text-xs bg-white text-right font-mono font-bold outline-none ring-2 ring-indigo-500/20"
                                                            onKeyDown={e => {
                                                    ...
                                                    >
                                                        ...
                                                        <Edit3 className="w-3 h-3 text-slate-300 group-hover/edit:text-indigo-600 transition-colors" />
                                                    </div>
                                                )}
                                            </td>
                                            <td className="px-5 py-3.5">
                            ...
                            <tfoot className="bg-slate-50/90 border-t-2 border-slate-200/80">
                                <tr>
                                    ...
                                    <td className="px-5 py-4 font-black text-right font-mono text-indigo-700 text-sm" colSpan={2}>
                                        {formatRp(totalNilai)}
                                    </td>
                                </tr>
                            </tfoot>
                            ...
                            <>
                                <div>
                                    ...
                                    <div className="flex items-center gap-2">
                                        <input 
                                            ...
                                            className="flex-1 px-3.5 py-2.5 bg-slate-50/80 border border-slate-200 rounded-xl text-xs sm:text-sm font-mono font-bold focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none transition-all"
                                            placeholder="0.0"
                                        />
                                ...
                                <div className="flex gap-2.5 pt-3 border-t border-slate-100">
                                    ...
                                    <button
                                        ...
                                        className="flex-1 px-4 py-2 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 text-white rounded-xl text-xs font-bold shadow-md shadow-indigo-500/20 disabled:opacity-50 cursor-pointer transition-all"
                                    >
                                        Verifikasi Selisih →
                                    </button>
                                </div>
                            ...
                            <>
                                <div className="bg-amber-50/80 border border-amber-200/80 rounded-2xl p-4 text-xs space-y-2">
                                    ...
                                    <p className="text-amber-800 leading-relaxed font-medium">
                                        ...
                                        <strong className="font-mono text-indigo-700">{newQtyNum.toLocaleString("id-ID")}</strong> {koreksiModal.product.display_unit || koreksiModal.product.unit}.
                                    </p>
